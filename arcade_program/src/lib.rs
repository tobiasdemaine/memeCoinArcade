...
use spl_token; // Import SPL Token crate

#[derive(BorshSerialize, BorshDeserialize, Debug, Clone, PartialEq)]
pub struct GameState {
    pub admin: Pubkey,
    pub high_scores: [(Pubkey, u64); 20],
    pub game_count: u64,
    pub prize_pool: u64,
    pub token_account: Pubkey,
    pub cost_to_play: u64,
    pub min_time_seconds: u32,
    pub active_sessions: Vec<(Pubkey, Hash, u64)>,
...
impl Pack for GameState {
    const LEN: usize = 1024 + 2048; // Rough estimate
    fn pack_into_slice(&self, dst: &mut [u8]) {
        let data = self.try_to_vec().expect("Failed to serialize GameState");
...
impl IsInitialized for GameRegistry {
    fn is_initialized(&self) -> bool {
        !self.games.is_empty() // Consider initialized if it has games
    }
}
...
impl Pack for GameRegistry {
    ...
    fn pack_into_slice(&self, dst: &mut [u8]) {
        let data = self.try_to_vec().expect("Failed to serialize GameRegistry");
        dst[..data.len()].copy_from_slice(&data);
    }
...
pub fn process_instruction(
    ...
    let _cos_acc = next_account_info(accounts_iter)?; // Unused COS account
    ...
    match instruction_data[0] {
        0 => {
            ...
            game_state.pack_into_slice(&mut game_state_acc.data.borrow_mut());
        ...
        4 => {
            ...
            registry.pack_into_slice(&mut registry_acc.data.borrow_mut());
